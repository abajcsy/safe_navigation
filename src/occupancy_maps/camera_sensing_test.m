...
indicies_inside_fov = find(abs(angle_to_source - 0.5*pi) <= half_fov);
...
angle_for_obs = angle_to_source(obs_indicies_of_interest) - 0.5*pi;
max_stretch = min(max(angle_for_obs), half_fov);
min_stretch = max(min(angle_for_obs), -half_fov);
indicies_to_trace = find(((angle_to_source - 0.5*pi) <= max_stretch) .* ...
  ((angle_to_source - 0.5*pi) >= min_stretch) .* (obs > 0));
...
num_alphas = 10;
alphas = linspace(0.01, 1, num_alphas);
...
for i=1:num_alphas
  ...
  error = abs(flattened_grid_x - points_x) + abs(flattened_grid_y - points_y);
