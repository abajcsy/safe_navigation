function [data, tau, extraOuts] = ...
    HJIPDE_solve_localQ_visualize(data0, lxOld, lx, updateEpsilon, tau, ...
    schemeData, compMethod, extraArgs, initVxColor, nextVxColor, figh, xcurr, ...
    next_belief_obstacle, nextGrid2D, init_obs)
...
%[seth, funch] = plotValueFun(data0, xcurr, currColor, schemeData, [], []);
initialData = data0;
% ================== %
...
[nh, qh, bh, seth, vout] = plotLocalQ(initialData, xcurr, currColor, ...
    Q, neighbors, lx, schemeData, nextGrid2D, ...
    next_belief_obstacle, init_obs, [], [], [], [], []);

% plot 3D function.
% [figh, funh, funh2, sh, q3h] = plotValueFun3D(initialData, xcurr, schemeData, ...
%     currColor, Q, neighbors, [], [], [], [], []);
% ================== %
...
for i = istart:length(tau)
    ...
    while tNow < tau(i) - small && ~isempty(Q) && (~isempty(setdiff(Q, Qold)) || ~isempty(setdiff(Qold, Q)))
        ...
        % ==== plotting ==== %
        currColor = [colorR(ii), colorG(ii), colorB(ii)];
        %[seth, funch] = plotValueFun(reshape(y, g.shape), xcurr, currColor, ...
        %    schemeData, seth, funch);
        currData = reshape(y, g.shape);
        
        % plot localQ algorithm.
        [nh, qh, bh, seth, vout] = plotLocalQ(currData, xcurr, currColor, ...
            Q, neighbors, lx, schemeData, nextGrid2D, next_belief_obstacle, ...
            init_obs, nh, qh, bh, seth, vout);
        
        % plot 3D function.
        %[figh, funh, funh2, sh, q3h] = plotValueFun3D(currData, xcurr, schemeData, ...
        %    currColor, Q, neighbors, figh, funh, funh2, sh, q3h);
        % ================== %
...
function [seth, funch] = plotValueFun(data0, xcurr, vxColor, schemeData, seth, funch)
    if ~isempty(seth)
        delete(seth);
    end
    if ~isempty(funch)
    ...
    %[grid2D, data2D] = proj(schemeData.grid, data0, [0 0 1 1], [xcurr(3),xcurr(4)]);
    [grid2D, data2D] = proj(schemeData.grid, data0, [0 0 1], [xcurr(3)]);
    extraArgs.LineWidth = 2;
    seth = visSetIm(grid2D, data2D, vxColor, 0, extraArgs);
    xticks([]);
    yticks([]);
    %funch = visFuncIm(grid2D, data2D, vxColor, 0.5);
    %xlim([-2,4]);
    %ylim([-3,1.5]);
    %c = camlight;
...
function [figh, funh, funh2, sh, q3h] = plotValueFun3D(data0, xcurr, schemeData, ...
    vxColor, Q, neighbors, figh, funh, funh2, sh, q3h)
    if isempty(figh)
        figh = figure(4);
    end
    if ~isempty(funh)
    ...
    % Plot 3D function (zero level).
%     extraArgs.alpha = 0.7;
%     funh = visSetIm(schemeData.grid,data0,vxColor,0,extraArgs);
%     
%     % Plot 3D function (-oneth level).
%     extraArgs.alpha = 0.3;
%     funh2 = visSetIm(schemeData.grid,data0,vxColor,-2,extraArgs);
    
    % Plot 2D zero level set. 
    levels = [0,0];
    levels = [-10:0.5:10];
    vxBoundaryColor = vxColor*(1 - 0.4);
    [grid2D, data2D] = proj(schemeData.grid, data0, [0 0 1], xcurr(3));
%     [~, sh] = contour(grid2D.xs{1}, grid2D.xs{2}, data2D, levels, ...
%         'linecolor', vxBoundaryColor, 'linewidth', 3);
    [~, sh]= contourf(grid2D.xs{1}, grid2D.xs{2}, data2D, levels, ...
        'linecolor', vxBoundaryColor, 'linewidth', 1.5, 'linestyle', 'none');
    rCol = linspace(vxColor(1), 1, 100)';
    gCol = linspace(vxColor(2), 1, 100)';
    bCol = linspace(vxColor(3), 1, 100)';
    cmap = horzcat(rCol, gCol, bCol);
    colormap(cmap);
    sh.ContourZLevel = 0;
    
    % Get the 2D grid.
    qGrid = -ones(size(data0));
    neighGrid = -ones(size(data0));
    
    % Get the Q and neighbor grid.
    qGrid(Q) = 1;
    %neighGrid(neighbors) = 1;
    shade_factor = 0.4;
    color = vxColor*(1 - shade_factor);
    extraArgs.alpha = 0.2;
    q3h = visSetIm(schemeData.grid,qGrid,[1,0,0],0,extraArgs);
    material dull
    
    % ------ TEST ------ %
%     figure(5)
%     [grid2D, data2D] = proj(schemeData.grid, qGrid, [0 0 1], 0);
%     surf(grid2D.xs{1}, grid2D.xs{2}, data2D,'FaceAlpha','flat',...
%     'AlphaDataMapping','scaled',...
%     'AlphaData',gradient(qGrid),...
%     'FaceColor','blue')
    % ------------------ %
    ...
    xlabel('$p_x$', 'Interpreter', 'latex', 'fontsize', 25);
    ylabel('$p_y$', 'Interpreter', 'latex', 'fontsize', 25);
    zlabel('$\theta$', 'Interpreter', 'latex', 'fontsize', 25);
    pause(0.05);
end

%% Plotting LocalQ Functionality.
function [ax0, ax1, ax2, seth, vout] = plotLocalQ(data0, xcurr, vxColor, ...
    Q, neighbors, lx, schemeData, nextGrid, nextObs, initObs, ...
    ...
    % Colors.
    qColor = [255, 0, 88]/255.;
    qOutline = [216, 0, 74]/255.;
    neighColor = [255, 155, 189]/255.;
    neighOutline = [255, 140, 179]/255.;
    
    % Get the 2D grid.
    qGrid = -ones(size(lx));
    neighGrid = -ones(size(lx));
    sz = size(Q);
    fprintf('Q: %f\n', sz(1));
    ...
    % Interpolate.
    fmm_dQ = compute_fmm_map(gQ, dQ); 
    interp_dQ = interp2(gQ.xs{1}', gQ.xs{2}', fmm_dQ', g.xs{1}, g.xs{2});
    ...
    fmm_dN = compute_fmm_map(gN, dN); 
    interp_dN = interp2(gN.xs{1}', gN.xs{2}', fmm_dN', g.xs{1}, g.xs{2});
...
%     shade_factor = 0.2;
%     color = vxColor*(1 - shade_factor);
%     [c, nh] = contourf(ax0, gN.xs{1}, gN.xs{2}, dN, [0,0], 'color', color);
%     colormap(ax0, color);
%     ax0.Visible = 'off';
...
%     levels = [-10:0.5:10];
%     %levels = [0,0];
%     [grid2D, data2D] = proj(schemeData.grid, data0, [0 0 1], 'max');
%     contour(seth, grid2D.xs{1}, grid2D.xs{2}, data2D, levels, ...
%         'linecolor', vxColor, 'linewidth', 2);
%     seth.Visible = 'off';
    seth = axes;
    levels = [-10:0.4:10];
    vxBoundaryColor = vxColor*(1 - 0.4);
    ...
    % Plot zero level of value function.
    hold on
    ax1 = axes;
    color = vxColor;
    [c, vh] = contour(ax1, gN.xs{1}, gN.xs{2}, data2D, [0,0], ...
    ...
    xticks([]);
    yticks([]);
    %legend([vh, oh, nh], {'zero-level', 'belief obs', 'states to update'});
